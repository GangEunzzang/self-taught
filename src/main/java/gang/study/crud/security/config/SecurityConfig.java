    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests() // 인가 설정
                ...
                // AuthenticationManager 란?
                    .loginProcessingUrl("/login") // // loginPage의 액션 POST값이 들어갈 URL 값
                    .defaultSuccessUrl("/list") // 로그인 성공시 이동 페이지
                    .failureUrl("/join") // 로그인 실패
                    .and()

                .logout()
                    .logoutRequestMatcher(new AntPathRequestMatcher("/logout"))
                    .logoutSuccessUrl("/list") // 로그아웃 성공시 연결되는 페이지
